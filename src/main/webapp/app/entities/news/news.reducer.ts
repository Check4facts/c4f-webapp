...
export const getSearchEntities: ICrudSearchAction<INews> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_NEWS,
  payload: axios.get<INews>(`${apiSearchUrl}?query=${query}${sort ? `&page=${page}&size=${size}&sort=${sort}` : ''}`),
});
