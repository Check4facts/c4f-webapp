...
import { getSearchEntities, getEntities } from './resource.reducer';
  ...
  return (
    <div>
      <h2 id="resource-heading">
        <Translate contentKey="check4FactsApp.resource.home.title">Resources</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="check4FactsApp.resource.home.createLabel">Create new Resource</Translate>
        </Link>
      </h2>
      <Row>
        <Col sm="12">
            ...
            <AvGroup>
              <InputGroup>
                <AvInput
                  ...
                  placeholder={translate('check4FactsApp.resource.home.search')}
                />
        ...
        {resourceList && resourceList.length > 0 ? (
            ...
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('url')}>
                  <Translate contentKey="check4FactsApp.resource.url">Url</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('harvestIteration')}>
                ...
                <th className="hand" onClick={sort('body')}>
                  <Translate contentKey="check4FactsApp.resource.body">Body</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('harvestDate')}>
                  <Translate contentKey="check4FactsApp.resource.harvestDate">Harvest Date</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="check4FactsApp.resource.statement">Statement</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {resourceList.map((resource, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{resource.body}</td>
                  <td>{resource.harvestDate ? <TextFormat type="date" value={resource.harvestDate} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{resource.statement ? <Link to={`statement/${resource.statement.id}`}>{resource.statement.id}</Link> : ''}</td>
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${resource.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      >
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${resource.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
                      >
                        <FontAwesomeIcon icon="trash" />{' '}
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="check4FactsApp.resource.home.notFound">No Resources found</Translate>
            </div>
          )
        )}
      </div>
