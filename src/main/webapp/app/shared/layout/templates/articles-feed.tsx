      ...
      {articles && articles.length > 0
        ? articles.map(article => (
            <Col sm="6" lg="3" className="mb-5 mb-sm-2 grid-margin" key={article.id}>
              ...
              <p className="fs-15 font-weight-normal">{article.previewText}</p>
              <div className="d-flex justify-content-between mb-3">
                <div>
                  <span className="fs-12 mr-1 text-muted">
                    {translate('check4FactsApp.statement.author')}: {(article.statement && article.statement.author) || 'N/A'}
                  </span>
                </div>
                <div>
                  <a
